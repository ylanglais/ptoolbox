...
$q = new oqry($db, "select count(*) as COUNT from CRM.LFM_PERSONNE");
$nn = $q->obj()->COUNT;
printf("Lignes:              %7d\n", $nn);

$q = new oqry($db, "select count(*) as COUNT from CRM.LFM_PERSONNE where MAIL is null");
$vides = $q->obj()->COUNT;
printf("Lignes sans email:   %7d\n", $vides);

$q = new oqry($db, "select count(*) as COUNT from CRM.LFM_PERSONNE where MAIL is not null");
$tocheck = $q->obj()->COUNT;
printf("Lignes à vérifer:    %7d\n", $tocheck);

$q = new oqry($db, "select PRS_ID, NOM, NOM_PRENOM, DATE_NAI, CP_C, TEL_POR, MAIL from CRM.LFM_PERSONNE where MAIL is not null");
...
while ($o = $q->obj()) {
	...
	if (!filter_var($o->MAIL, FILTER_VALIDATE_EMAIL)) {
		if (filter_var(preg_replace("/ /", "", $o->MAIL), FILTER_VALIDATE_EMAIL)) {
			$extra++;
		} else if (filter_var(preg_replace("/,/", ".", $o->MAIL), FILTER_VALIDATE_EMAIL)) {
			$coma++;
		} else if (preg_match("/[A-Za-z0-9._-]+@[A-Za-z0-9._-]+.[A-Za-z]*/", $o->MAIL)) {
			$maybe++;
		} else {
			$bad++;
		}
