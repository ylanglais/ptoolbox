...
function api_notfound($reason = "") {
	...
	if ($reason == "") {
		print(json_encode([ "msg" => "Not found" ]));
	} else {
		print(json_encode([ "msg" => "Not found", "reason" => $reason ]));
	}	
	exit;
}
...
function api_auth_required($reason = "") {
	...
	if ($reason == "") {
		print(json_encode(["msg"=> "Network Authentication Required"]));
	} else {
		print(json_encode([ "msg" => "Network Authentication Required", "reason"=> $reason]));
	}
	exit;
}
...
function api_forbiden($reason = "") {
	...
	if ($reason == "") {
		print(json_encode([ "msg" => "Forbidden" ]));
	} else {
		print(json_encode([ "msg" => "Forbidden", "reason" => $reason ]));
	}	
	exit;
}
function api_reply($msg, $data = '', $token = false) {
	if ($msg != "welcome" && $msg != "ok" && $msg != "goodby") {
		#_err("msg: $msg ($data)");
		print(json_encode( [ "msg" => $msg, "reason" => $data ]));
		exit;
	} 
	if ($token !== false) header("Authorization: Bearer $token");
	#header('Content-type: application/json');
	print(json_encode([ "msg"=> $msg, "data" => $data ]));
	exit;
}
#
